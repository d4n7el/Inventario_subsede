<?php 
	...
	$fecha = date('Y-m-d');
	$tomorrow = date( "Y-m-d", strtotime( "+4 day", strtotime($fecha)));  
	...
	if (isset($_SESSION["id_user_activo"])) {
		...
		$limit = 40;
		$offset = $limit * $pagina;
		$retorno_count_exit = $count_exits->get_exit_equipments_count($team,$cedula,$fecha_inicial,$fecha_final,$estado);
		$count_rows = $retorno_count_exit['count'];
		$href = '/php/equipments/index_exit.php';
		// // NECESARIO PARA LA PAGINACION
		 $retorno_exit = $exits->get_exit_equipments($team,$cedula,$fecha_inicial,$fecha_final,$estado,$limit,$offset);

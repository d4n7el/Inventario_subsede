...
foreach ($retorno_equipos as $key => $value) { ?>
	...
	<div class="row" id="update_<?php echo $value['id_user'] ?>">
		<h6 class="titulo col s12 fondo_negro color_letra_primario center paddin1">		
			Editar Cantidad disponible del Equipo.
		</h6>
		...
		<section>
			<form action="<?php $_SERVER['DOCUMENT_ROOT'] ?>/php/equipments/update_equipment.php" class="update_info" accept-charset="utf-8">
		        ...
		        <div class="input-field col s4">
		        	<i  class="material-icons prefix">business_center</i>
		        	<input id="cantidad_disponible" type="number" class="validate editar_info" name="cantidad_disponible" autocomplete="off" min="0" value="<?php echo $value['quantity_available']+ $retorno_outside['count']?>" require>
		        	<label for="cantidad_disponible" class="active">Cantidad disponible</label>
		        </div>
